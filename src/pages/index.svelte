<script lang="ts">
    ...
    const handleSubmit = (item) => {
       CartStore.update((items) => [...items,  {dishId:item.dishId,dishname:item.dishname, price:item.price,description:item.description,quantity:1, image:item.image}]);
  ...
  {#each $DishInfoStore as item}
    ...
    <form on:submit|preventDefault={() => handleSubmit(item)} action=""> 
      ...
      <div class="px-6 py-4">
        ...
        <a href="{$url('../dishdetails/?id=' + item.dishId )}" class="bg-gray-900 text-white px-3 py-1 mt-3 rounded-lg hover:bg-yellow-500 transition duration-300 ease-in-out">
          View Details
