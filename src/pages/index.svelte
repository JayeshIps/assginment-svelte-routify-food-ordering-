...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:mx-5 md:mr-18 mx-3">
  {#each $DishInfoStore as item}
    <div class="card bg-white rounded-lg overflow-hidden shadow-md text-center transition duration-300 ease-in-out hover:bg-gray-100 hover:shadow-lg hover:text-gray-900 hover:border-gray-900 border-2">
      <img src="{item.image}" alt="" class="w-full h-32 object-cover">
      <div class="px-6 py-4">
        
        <div class="font-bold text-lg mb-2">{item.dishname}</div>
